...
detach(blood)
###########omite the influence#######
x=blood[-c(8,32,204,231,243,339,355,356,366,375,403,474,485),c(2:18)]
attach(blood)
y=x[,1]
y=as.numeric(y)
fit.lin.omit=lm(y~x$gender+x$married+x$smoke+as.factor(x$exercise)+x$age+x$weight+x$height+
             as.factor(x$overwt)+as.factor(x$race)+as.factor(x$alcohol)+as.factor(x$trt)
           +x$bmi+as.factor(x$stress)+as.factor(x$salt)+
             as.factor(x$chldbear)+as.factor(x$income)+as.factor(x$educatn))
summary(fit.lin.omit)
